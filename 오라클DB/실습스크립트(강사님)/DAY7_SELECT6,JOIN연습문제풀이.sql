...
--컬럼명 > (값1, 값2, 값3, ...) => 컬럼명 > 값1 OR 컬럼명 > 값2 OR 컬럼명 > 값3 ...

--예 : 대리 직급의 직원 중 과장 직급의 급여의 최소값보다 급여를 많이 받는 직원 조회
...
--컬럼명 > (값1, 값2, 값3, ...) => 컬럼명 > 값1 AND 컬럼명 > 값2 AND 컬럼명 > 값3 ...
...
FROM EMPLOYEE
LEFT JOIN JOB USING (JOB_ID)
WHERE SALARY IN (SELECT JOB_ID, TRUNC(AVG(SALARY), -5)
                                    FROM EMPLOYEE
                                    GROUP BY JOB_ID);

-- FROM 절에서 서브쿼리 사용가능 : 테이블 대신에 사용
-- FROM (서브쿼리) 별칭
-- FROM 절에서 사용된 서브쿼리가 만든 결과집합을
-- 인라인뷰(INLINE VIEW) 라고 함
SELECT EMP_NAME, JOB_TITLE, SALARY
...
-- 서브쿼리의 종류
-- 단일행, 다중행, 다중행 다중열, 상[호연]관, 스칼라 서브쿼리
-- 대부분의 서브쿼리는 서브쿼리가 만들어 낸 결과값을 메인쿼리가 사용하는 방식
-- 상관쿼리는 서브쿼리가 메인쿼리의 값을 가져다가 결과를 만드는 구조
-- 그러므로 메인쿼리의 값이 바뀌면 서브쿼리의 결과도 달라짐 => 상호연관 서브쿼리

-- 자기 직급의 평균 급여를 받는 직원 조회 : 상호연관 서브쿼리 사용
SELECT EMP_NAME, JOB_TITLE, SALARY
FROM EMPLOYEE E
LEFT JOIN JOB J ON (E.JOB_ID = J.JOB_ID)
...
-- 상호연관 서브쿼리 문 앞에서만 사용가능
-- 서브쿼리가 만든 결과가 존재하는지 물어볼 때는 EXISTS 사용
-- 서브쿼리의 SELECT 절에 컬럼 기술하면 안됨. NULL 사용할 것
...
-- 부서의 근무지역이 'OT'이면 '지역팀', 아니면 '본사팀' 으로 표시 후 조회
SELECT EMP_ID, EMP_NAME,
...
-- ORDER BY 절에 스칼라 서브쿼리 사용 예(잘 사용되진 않음)
...
-- TOP-N 분석 ---------------------------------------------------------------
-- 상위 몇 개, 하위 몇 개를 조회하는 것 (2가지 방법)
-- 1. 인라인 뷰와 RANK() 함수를 이용하는 TOP-N 분석
-- 2. ROWNUM 을 이용한 TOP-N 분석

-- 1. 인라인 뷰와 RANK() 함수를 이용하는 TOP-N 분석
-- 직원 정보에서 급여를 가장 많이 받는 직원 5명(TOP-5) 조회
-- 이름, 급여, 순위
SELECT *
...
-- 2. ROWNUM 을 이용한 TOP-N 분석
-- ORDER BY 한 결과에 ROWNUM 을 붙임 => 서브쿼리 이용
-- ROWNUM : 행 번호를 의미, WHERE 처리 후에 자동으로 부여
...
-- 급여 많이 받는 직원 3명 조회(TOP-3)
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM < 4 --WHERE 절 다음에 ROWNUM 부여
ORDER BY SALARY DESC; --틀린 결과 나옴

-- 해결 : 정렬하고 나서 ROWNUM 이 부여되게끔 질의
-- 인라인 뷰 사용
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT * 
            FROM EMPLOYEE
            ORDER BY SALARY DESC) --정렬 후에 ROWNUM 부여
WHERE ROWNUM < 4;